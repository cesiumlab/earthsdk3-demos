...
<template>
    <RightList :title="'可视化--谷歌影像'">
        <div class="roam_moveOnLineMode" style="margin-left: 15px;">
            <div>
                <label for="">时间:</label><input type="text" placeholder="请选择" v-model="currentTime" readonly>
                <span class="roam_listSpan" @click="ulIsShow = !ulIsShow"><es-icon :name="'liebiao'"
                        :color="'rgba(216, 216, 216, 1)'" :size="15" /></span>
                <ul v-if="ulIsShow" class="analysis_ul" :key="a">
                    <li v-for="item in timesList" @click="changeLayerListShow(item)" :key="item">{{ item }}</li>
                </ul>
            </div>
        </div>
    </RightList>
