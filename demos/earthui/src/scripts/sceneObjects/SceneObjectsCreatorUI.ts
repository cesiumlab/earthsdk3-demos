...
import {
  Destroyable,
  PartialWithUndefinedReactivePropsToNativeProps,
...
function createDivClass(sceneObject: SceneObjectsCreatorUI) {
  return class MyDiv extends Destroyable {
    ...
    get sceneTreeApp() {
      return this._sceneTreeApp
    }

    constructor(
      subContainer: HTMLDivElement,
      customDiv: ESCustomDiv,
...

      {
        const update = () => {
          ...
          this._sceneTreeApp._container._vnode.component.props.show =
            sceneObject.showSceneObjectsCreatorRef
        }
        update()
...
export class SceneObjectsCreatorUI extends ESSceneObject {
  static readonly type: string = this.register('SceneObjectsCreatorUI', this, {
    chsName: 'SceneObjectsCreatorUI',
    tags: ['MiscObjects'],
    description: 'SceneObjectsCreatorUI'
  })
  get typeName() {
    return 'SceneObjectsCreatorUI'
  }
  override get defaultProps() {
    return SceneObjectsCreatorUI.createDefaultProps()
  }
  get json() {
    return this._innerGetJson() as JsonType
  }
  ...
  get sceneObjectTypeGetting() {
    return this._sceneObjectTypeGetting
  }

  private _customDiv = this.disposeVar(new ESCustomDiv())
  get customDiv() {
    return this._customDiv
  }
  ...
  get showSceneObjectsCreatorRef() {
    return this._showSceneObjectsCreatorRef.value
  }
  set showSceneObjectsCreatorRef(value: boolean) {
    this._showSceneObjectsCreatorRef.value = value
  }
  get showSceneObjectsCreatorRefChanged() {
    return this._showSceneObjectsCreatorRef.changed
  }
  ...
  get containerChanged() {
    return this._container.changed
  }
  ...
  private _processing = this.disposeVar(
    createProcessingFromAsyncFunc(async (cancelsManager) => {
  ...
  constructor(id?: string) {
    ...
    this.dispose(
      bind([this, 'showSceneObjectsCreatorRef'], [this.sceneObjectTypeGetting, 'showDialog'])
    )
    ...
    return [
      ...
      new GroupProperty('通用', '通用', [
        new StringProperty(
          ...
          false,
          [this, 'containerId'],
          SceneObjectsCreatorUI.defaults.containerId
        ),
...
export interface SceneObjectsCreatorUI extends ReactivePropsToNativePropsAndChanged<
  ReturnType<typeof SceneObjectsCreatorUI.createDefaultProps>
> {}
type JsonType = PartialWithUndefinedReactivePropsToNativeProps<
  ReturnType<typeof SceneObjectsCreatorUI.createDefaultProps> & { type: string }
>
