...
import { ESCylinder } from 'earthsdk3'
...
const modes = [
    ...
    {
        mode: 'policeCar',
        img: new URL('../../../assets/plotting/esCylinder.png', import.meta.url).href,
        name: '圆柱体'
    }
]
...
let sceneObject: ESCylinder | undefined = undefined
let sceneObject2: ESCylinder | undefined = undefined
...
const createSceneObject = (position: [number, number, number]) => {
    ...
    sceneObject2 = xbsjEarthUi.createSceneObject(ESCylinder) as ESCylinder
    sceneObject2.position = position
    // sceneObject2.mode = selected.value.mode
    const sceneObjectIndex = getsceneObjNumfromSceneTree(xbsjEarthUi, 'ESCylinder')
    setTimeout(() => {
        //@ts-ignore
...
const createOneSceneObject = () => {
    if (!selected.value) return
    sceneObject = xbsjEarthUi.createSceneObject(ESCylinder) as ESCylinder
    if (sceneObject) {
