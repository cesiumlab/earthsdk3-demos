        ...
        <div class="images_img_list">
            <div v-for="(item, index) in modes" class="images_img_lilist">
                <div class="images_imgposition" @click="select(item)"
                    :class="{ 'images_checkedactive': selected && item.materialMode === selected.materialMode }">
...
let sceneObject: ESPipeFence | undefined = undefined
const select = (item: { materialMode: string, img: any, name: string, }) => {//点击选择框中的折线按钮
    destroy()
    selected.value = item
    createSceneObject()
}
...
const createSceneObject = () => {
    ...
    sceneObject = xbsjEarthUi.createSceneObject(ESPipeFence) as ESPipeFence
    if (sceneObject) {
        const sceneObjectIndex = getsceneObjNumfromSceneTree(xbsjEarthUi, 'ESPipeFence')
