<template>
    <PopList :title="'关卡包'" :showButton="true" @close="cancel" @ok="ok">
        <LabelInput v-model="url" :label="'路径'"></LabelInput>
        <LabelInput v-model="name" :label="'名称'" :placeholder="'非必填'"></LabelInput>
...
const ok = () => {
    ...
    if (!lastSceneTreeItem) {
        treeItem = sceneTree.createSceneObjectTreeItem(ESLevelRuntimeModel)
    } else {
        if (lastSceneTreeItem.type === 'Folder') {
            treeItem = sceneTree.createSceneObjectTreeItem(ESLevelRuntimeModel, undefined, lastSceneTreeItem, 'Inner')
        } else {
            treeItem = sceneTree.createSceneObjectTreeItem(ESLevelRuntimeModel, undefined, lastSceneTreeItem, 'After')
        }
    }
    ...
    const sceneObjectIndex = getsceneObjNumfromSceneTree(xbsjEarthUi, 'ESLevelRuntimeModel')
    sceneObject.position = position.value
    sceneObject.name = (name.value ?? '关卡包模型') + (sceneObjectIndex + 1)
    if (name.value) {
        sceneObject.levelName = name.value
    }
    sceneObject.levelLoadDistance = 100000
    emits('close')
    destroy()
}
const cancel = () => {
