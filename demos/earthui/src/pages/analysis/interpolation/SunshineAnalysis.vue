...
onMounted(() => {
  ...
  dispose = sunshineAnalysis.editingChanged.disposableOn(() => {
    if (sunshineAnalysis.editing === false) {
      sampleDistance.value = Number(
        Math.sqrt(sunshineAnalysis.area / xbsjEarthUi.Interpolation2).toFixed(2)
      )
  ...
  dispose2 = sunshineAnalysis.dispose(
    sunshineAnalysis.progressChanged.disposableOn((res: any) => {
      if (res === 100) {
        flag.value = true
      }
    })
