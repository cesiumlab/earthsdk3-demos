...
let sceneObject: ESFireParticleSystem | undefined = undefined
let sceneObject2: ESFireParticleSystem | undefined = undefined
...
const createSceneObject = (position: [number, number, number]) => {
  ...
  sceneObject2 = xbsjEarthUi.createSceneObject(ESFireParticleSystem) as ESFireParticleSystem
  sceneObject2.position = position
  sceneObject2.image = selected.value.imgUrl
  const sceneObjectIndex = getsceneObjNumfromSceneTree(xbsjEarthUi, 'ESFireParticleSystem')
  setTimeout(() => {
    //@ts-ignore
...
const createOneSceneObject = () => {
  if (!selected.value) return
  sceneObject = xbsjEarthUi.createSceneObject(ESFireParticleSystem) as ESFireParticleSystem
  if (sceneObject) {
