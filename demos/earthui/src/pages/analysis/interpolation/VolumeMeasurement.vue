<template>
    ...
    <PopList :title="'体积测量'" :showButton="true" @close="restart" @ok="start" :leftButton="'重新绘制'" :rightButton="'开始分析'"
        ...
        <LabelInput v-model="fillAreaRef" :inputType="'number'" :label="'填方'" :unit="'m³'" :readonly="true">
        </LabelInput>
        <LabelInput v-model="CutAndFillAreaRef" :inputType="'number'" :label="'挖填方'" :unit="'m³'" :readonly="true">
        </LabelInput>
        <LabelInput v-model="schedule" :inputType="'number'" :label="'计算进度'" :unit="'%'" :readonly="true"></LabelInput>
    </PopList>
</template>
