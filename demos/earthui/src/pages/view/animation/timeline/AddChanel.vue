    ...
    <div class="animation_list">
      <div class="animation_list_left" style="border-right: 1px solid rgba(183, 183, 183, 0.5)">
        ...
        <div class="animation_list_list">
          <div class="animation_list_item" v-for="item in esPathList">
            <span class="animation_list_item_span" @click="itemPathCheck(item)">
              <es-icon
      ...
      <div class="animation_list_left">
        <div class="animation_list_title">
          <span
            class="develop_icon"
            :class="{ 'disabled-element': esPathFlagLength === 0 }"
            @click.stop="allLocationCheck"
            :title="esLocationCheckBox ? '取消全选' : '全选'"
          >
            <es-icon
              :name="esLocationCheckBox ? 'checked' : 'unchecked'"
              :color="
                esPathFlagLength === 0
                  ? '#706b6b'
                  : esLocationCheckBox
        ...
        <div class="animation_list_list">
          <div
            class="animation_list_item"
            :class="{ 'disabled-element': esPathFlagLength === 0 }"
            v-for="item in esLocationList"
          >
            <span
              class="animation_list_item_span"
              :class="{ 'disabled-element': esPathFlagLength === 0 }"
              @click="itemLocationCheck(item)"
            >
              <es-icon
                :name="item.flag ? 'checked' : 'unchecked'"
                :color="
                  esPathFlagLength === 0 ? '#706b6b' : item.flag ? '#fff' : 'rgba(230, 230, 230, 1)'
                "
                :size="16" /></span
            >{{ item.name }}
          </div>
...
const searchAndCollectLineValues = (obj: { [key: string]: any }, resultArray: any[]) => {
  if (obj['sceneObj']) {
    if (obj['sceneObj'].type && obj['sceneObj'].type === 'ESPath') {
      //ESPath
      resultArray.push({
...
//获取所有 点位对象
const searchAndCollectLocationValues = (
  xbsjEarthUi: any,
  obj: { [key: string]: any },
  resultArray: any[]
) => {
