<template>
  ...
  >
    <div class="lift_height">
      ...
      ></LabelInput>
      <div
        v-if="liftHeightType === 'blank' || liftHeightType === 'folder'"
        class="blank_lift_sceneObj"
      >
        当前的抬升对象有{{ blankLiftSceneObj }}个，符合抬升的对象有{{ rightLiftSceneObj }}个
      </div>
    </div>
  </Window>
</template>
...
import {
  searchCheckedFromFolders,
  searchCheckedTreeItems,
  searchSceneObjectFromFolders,
  searchSceneObjectTreeItems
} from '../tools'
...
const sceneObjectLiftHeight = (sceneObject: any) => {
  //对象抬升高度
  if (sceneObject instanceof ESObjectWithLocation) {
    ...
    const position = [sceneObject.position[0], sceneObject.position[1], newHeight] as [
      number,
      number,
      number
    ]
