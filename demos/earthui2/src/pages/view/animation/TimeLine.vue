<template>
    <div class="time" ref="time">
        ...
        <div class="control_play">
            <div class="control_currenttime">{{ timestampToTime(currentTime, true) }}</div>
            ...
            <div class="control_buton">
                ...
                <div class="control_buton_item" @mouseenter="hoverHighLight = 2" @mouseleave="hoverHighLight = -1"
                    ...
                    <es-icon :name="'AB_1'"
                        :color="hoverHighLight === 2 || abRange ? 'rgba(44, 104, 247, 1)' : 'rgba(216, 216, 216, 1)'"
                        :size="12" />
                </div>
                <!-- 循环 -->
                <div class="control_buton_item" @mouseenter="hoverHighLight = 3" @mouseleave="hoverHighLight = -1"
                    @click="loopChange" :class="{ 'disabled-element': !abRange }"><es-icon :name="'xunhuan_1'"
                        :color="hoverHighLight === 3 || loop ? 'rgba(44, 104, 247, 1)' : 'rgba(216, 216, 216, 1)'"
                        :size="12" />
                </div>
                <!-- 播放 -->
                <div class="control_buton_item" @mouseenter="hoverHighLight = 4" @mouseleave="hoverHighLight = -1"
                    @click="playing = !playing">
                    <es-icon :name="playing ? 'zanting_1' : 'bofang_1'"
                        :color="hoverHighLight === 4 || playing ? 'rgba(44, 104, 247, 1)' : 'rgba(216, 216, 216, 1)'"
                        :size="12" />
                </div>
            </div>
            <div class="control_buton">
                ...
                <div class="control_buton_item" @mouseenter="hoverHighLight = 5" @mouseleave="hoverHighLight = -1"
                    ...
                    <es-icon :name="'tongbu_weixuanzhong'"
                        :color="hoverHighLight === 5 || xbsjEarthUi.activeViewer?.timeSync ? 'rgba(44, 104, 247, 1)' : 'rgba(216, 216, 216, 1)'"
                        :size="14" />
                </div>
            </div>
        </div>
...
///////
//滑动速率滑块
function onRateDragMouseDown() { // 在滚动条上拖动左滑块
    //@ts-ignore
...
//动态修改整的dom元素的宽度
xbsjEarthUi.navigatorManager.dispose(xbsjEarthUi.navigatorManager.timeLineWidthChanged.disposableOn((val) => {
    if (time.value) {
