<template>
    <div class="player_barbox">
        ...
        <div class="player_barbox_top">
            ...
            <div class="control_list_button">
                <div class="control_buton_item" @mouseenter="hoverHighLight = 1" @mouseleave="hoverHighLight = -1"
                    @click="addChannel" title="添加"><es-icon :name="'tianjia_2'"
                        :color="hoverHighLight === 1 ? 'rgba(44, 104, 247, 1)' : '#fff'" :size="12" />
                </div>
                <div class="control_buton_item" :class="{ 'disabled-element': channelFlagLength < 1 }"
                    ...
                    <es-icon :name="'shanchu_1'"
                        :color="channelFlagLength < 1 ? 'rgba(230, 230, 230, 1)' : hoverHighLight === 2 ? 'rgba(44, 104, 247, 1)' : '#fff'"
                        :size="12" />
                </div>
                <div class="control_buton_item" :class="{ 'disabled-element': channelFlagLength !== 1 }"
                    @mouseenter="hoverHighLight = 3" @mouseleave="hoverHighLight = -1" @click="chanelItem" title="修改">
                    <es-icon :name="'bianji_weixuanzhong'"
                        :color="channelFlagLength !== 1 ? 'rgba(230, 230, 230, 1)' : hoverHighLight === 3 ? 'rgba(44, 104, 247, 1)' : '#fff'"
                        :size="11" />
                </div>
            </div>
            <!-- 上方时间线 -->
            <div class="time_player_scale">
                ...
                <!-- icon  B -->
                <div v-if="abRange"
                    :style="{ position: 'absolute', left: `${(stopTimeRef - playerStarttimeAxis) / 1000 * xScale - 6}px`, top: '22px' }">
                    <es-icon :name="'B-copy'" :color="'#fff'" :size="12" />
                </div>
                <!-- icon  A -->
                <div v-if="abRange"
                    :style="{ position: 'absolute', left: `${(startTimeRef - playerStarttimeAxis) / 1000 * xScale - 6}px`, top: '22px' }">
                    <es-icon :name="'A-copy'" :color="'#fff'" :size="12" />
                </div>
                <!-- icon  current -->
                <div
                    :style="{ position: 'absolute', left: `${(currentTime - playerStarttimeAxis) / 1000 * xScale - 6}px`, top: '22px' }">
        ...
        <div class="player_barbox_bottom">
            ...
            <!-- 总时间段背景 -->
            <div v-if="abRange"
                :style="{ position: 'absolute', left: `${(startTimeRef - playerStarttimeAxis) / 1000 * xScale + 104}px`, top: '0', width: `${(stopTimeRef - startTimeRef) / 1000 * xScale}px`, height: `${totalItemHeight}px`, background: 'rgba(121, 161, 255, 0.3)' }">
            </div>
            <!-- 下面的线段 -->
            ...
            <div class="time_player_list">
                <svg :width="totaltimeAxis * xScale" :height="totalItemHeight" preserveAspectRatio="none"
                    ...
                    <!-- 循环结束时间的线 -->
                    <line v-if="abRange" class="last-time-line"
...
import { CurrentTimeLineDragger, StartTimeLineDragger, Dragger, LastTimeLineDragger, PlayingRectLineDragger, PlayingChannelLeftLineDragger, PlayingChannelRightLineDragger } from './playerEditorTools';
...
const totalItemHeight = computed(() => 21.2 * (channels.value && channels.value.length > 5 ? channels.value.length : 5))//根据列表计算的总高
const textInfo = reactive({ x: 0, y: 0, show: false, text: '' });//划过时间的信息
const playerScaleList = computed(() => {//时间线的间隔
...
const channelsRef = ref<{//ui上的动画列表
    pathId: string;
    flag: boolean;
    sceneObjectIds: string[];
}[]>([])
watch(() => props.abRange, (val) => {
    if (val) {
...
watch(playerStarttimeAxis, (val) => {
    if (props.abRange) return
    startTime.value = val
}, { immediate: true })
watch(playerStoptimeAxis, (val) => {
    if (props.abRange) return
    stopTime.value = val
}, { immediate: true })

watch(startTimeRef, (val) => {
    if (!props.abRange) return
    startTime.value = val
...
const ok = (list: { pathId: string, sceneObjectIds: string[] }, flag: boolean) => {//点击确认添加
...
xbsjEarthUi.dispose(xbsjEarthUi.pathAnimationManager.channelsChanged.disposableOn((val) => {
    const v = [...val]
    let ch: {
        pathId: string;
        flag: boolean;
...
const pointerdown = (event: PointerEvent) => {
    ...
    if (name === 'startTimeLine') {
        ...
        const index = +(dom.getAttribute('index') ?? 0)
        currentDragger = new PlayingChannelLeftLineDragger(event, xScale, index, xbsjEarthUi, channelsRef);
    } else if (name === 'channel_right_line') {
        const index = +(dom.getAttribute('index') ?? 0)
        currentDragger = new PlayingChannelRightLineDragger(event, xScale, index, xbsjEarthUi, channelsRef);
    }
}
