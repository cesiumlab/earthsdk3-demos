...
// ______________________________________________________________________
//寻找节点上的所有对象
export function getSceneObjectsFromTreeItem(innerobjItem: SceneTreeItem, result: ESSceneObject[]) {
  if (innerobjItem.sceneObject) {
    result.push(innerobjItem.sceneObject)
  }
  if (innerobjItem.children) {
    for (let i = 0; i < innerobjItem.children.length; i++) {
      getSceneObjectsFromTreeItem(innerobjItem.children._innerObj[i] as SceneTreeItem, result)
    }
  }
}
//寻找场景树上的所有的对象
