<template>
  <div
    class="treeitem"
    :style="{ paddingLeft: 24 * item.level + 10 + 'px' }"
    @mouseenter="mouseEnter"
    @mouseleave="mouseLeave"
    @dblclick.stop="dbclickItem(item, $event)"
    :class="{ selected: isCurrentSelect }"
    @click.stop="selectItem(item)"
    @contextmenu="onItemContextMenu(item)"
  >
    <div class="item-content">
      <div class="expand" @dblclick.stop @click.stop="itemExpand(item)">
        <img
          src="../../../../assets/common/tree/expand.png"
          :class="[{ expandRotate: !item.expand }, { expandReset: item.expand }]"
      ...
      <input
        ...
        @change="setName(item)"
        :value="item.name"
        :class="{ 'input-focus': item.editing }"
...
export default {
  ...
  watch: {
    item: {
      handler(val) {
        this.checkedIcon = val.checked
          ? new URL('../../../../assets/common/tree/select.png', import.meta.url).href
          : new URL('../../../../assets/common/tree/unselect.png', import.meta.url).href
        if (val.editing) {
          this.$nextTick(() => {
