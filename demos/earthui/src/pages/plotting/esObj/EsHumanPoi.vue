...
import { ESHumanPoi } from "earthsdk3";
...
let sceneObject: ESHumanPoi | undefined = undefined
let sceneObject2: ESHumanPoi | undefined = undefined
...
const createSceneObject = (position: [number, number, number]) => {
    ...
    sceneObject2 = xbsjEarthUi.createSceneObject(ESHumanPoi) as ESHumanPoi
    sceneObject2.position = position
    sceneObject2.mode = selected.value.mode
    const sceneObjectIndex = getsceneObjNumfromSceneTree(xbsjEarthUi, 'ESHumanPoi')
    setTimeout(() => {
        //@ts-ignore
...
const createOneSceneObject = () => {
    if (!selected.value) return
    sceneObject = xbsjEarthUi.createSceneObject(ESHumanPoi) as ESHumanPoi
    if (sceneObject) {
