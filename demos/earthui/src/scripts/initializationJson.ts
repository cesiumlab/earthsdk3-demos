...
function initSceneId(xbsjEarthUi: XbsjEarthUi) {
  ...
  get(url, xbsjEarthUi.cesiumLabToken ? undefined : token)
    .then((res: any) => {
      if (res.code === 1000) {
    ...
    get(`${origin}/setting/get`)
      .then((res: any) => {
        if (res.status === 'ok') {
          ...
          if (sceneid) {
            get(`${origin}/staticscene/get?id=${sceneid}`, token)
              .then((res: any) => {
                if (res.status === 'ok') {
      ...
      .catch((err) => {
        get(`${origin}/staticscene/${sceneid}`, token)
          .then((res: any) => {
            if (res.status === 'ok') {
              ElMessage.success(`加载场景成功：${appid}`)
              const options = {
...
export function initSceneWithType(xbsjEarthUi: XbsjEarthUi) {
  ...
  if (!sceneTree) return
  if (parseSearch.type) {
    if (parseSearch.type === 'images') {
      //存在images
      const rectangle = [
        +(parseSearch.west ?? -180),
        +(parseSearch.south ?? -90),
        +(parseSearch.east ?? 180),
        +(parseSearch.north ?? 90)
      ]
      // console.log('images', parseSearch);
      const czmTilingScheme =
        parseSearch.proj === '4326' ? 'GeographicTilingScheme' : 'WebMercatorTilingScheme'
      const url =
        parseSearch.tiletrans === 'tms'
          ? `${parseSearch.url}/tilemapresource.xml`
          : `${parseSearch.url}`
      let newUrl
      if (xbsjEarthUi.cesiumLabToken) {
        newUrl = {
    ...
    } else if (parseSearch.type === 'terrains') {
      //存在terrains
      const rectangle = [
        +(parseSearch.west ?? -180),
        +(parseSearch.south ?? -90),
    ...
    } else if (parseSearch.type === 'models') {
      //存在models
      let newUrl
      if (xbsjEarthUi.cesiumLabToken) {
        newUrl = {
    ...
    } else if (parseSearch.type === 'vecImages') {
      //vecImages
