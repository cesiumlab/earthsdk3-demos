<template>
    <PopList :title="'中心旋转'">
        <LabelInput v-model="position[0]" :inputType="'number'" :label="'经度'" :max="180" :min="-180"></LabelInput>
        <LabelInput v-model="position[1]" :inputType="'number'" :label="'纬度'" :max="90" :min="-90"></LabelInput>
        ...
        <LabelInput v-model="orbitPeriod" :inputType="'number'" :label="'绕行周期'" :unit="'s'" :noZero="true"></LabelInput>
        <LabelInput v-model="distance" :inputType="'number'" :label="'相机距离'" :unit="'m'" :min="0"></LabelInput>
    </PopList>
</template>
...
const ueActorResult = () => {
    ...
    if (!viewer) return
    let disposeUe = viewer.clickEvent.donce(async e => { // ue中没有pointerDown，但是ue的click相当于pointerDown
...
watch([position, orbitPeriod, pitch, heading, distance], () => {
    const viewer = xbsjEarthUi.activeViewer
    if (!viewer) return
    viewer.changeToRotatePoint(position.value, distance.value, orbitPeriod.value, heading.value, pitch.value)
},{deep:true})
onMounted(() => {
    const viewer = xbsjEarthUi.activeViewer
    if (!viewer) return
