        ...
        <div class="images_img_list">
            <div v-for="(item, index) in modes" class="images_img_lilist">
                <div class="images_imgposition" @click="select(item)"
                    :class="{ 'images_checkedactive': selected && item.imgUrl === selected.imgUrl }">
...
const modes = [//多选模式类型
    {
        ...
        img: new URL('../../../assets/plotting/collision.png',import.meta.url).href,
        name: '粒子爆炸'
    }
]
...
let sceneObject: ESBlastParticleSystem | undefined = undefined
let sceneObject2: ESBlastParticleSystem | undefined = undefined
let editingDispose: any = undefined
let editingDispose2: any = undefined
let ececutDispose: any = undefined
const changeCheckBox = () => {//点击取消连续创建时使得粒子爆炸类型为空
...
const select = (item: { imgUrl: string, img: any, name: string, }) => {//点击选择框中的粒子爆炸按钮
    selected.value = item
    destroy()
...
const createSceneObject = (position: [number, number, number]) => {
    ...
    sceneObject2 = xbsjEarthUi.createSceneObject(ESBlastParticleSystem) as ESBlastParticleSystem
    sceneObject2.position = position
    sceneObject2.image = selected.value.imgUrl
    const sceneObjectIndex = getsceneObjNumfromSceneTree(xbsjEarthUi, 'ESBlastParticleSystem')
    setTimeout(() => {
        //@ts-ignore
...
const createOneSceneObject = () => {
    if (!selected.value) return
    sceneObject = xbsjEarthUi.createSceneObject(ESBlastParticleSystem) as ESBlastParticleSystem
    if (sceneObject) {
        sceneObject.image = selected.value.imgUrl
        const sceneObjectIndex = getsceneObjNumfromSceneTree(xbsjEarthUi, 'ESBlastParticleSystem')
