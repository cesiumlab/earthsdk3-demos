...
// 比例尺组合式函数
export function useScale(xbsjEarthUi: XbsjEarthUi, navigatorScaleRight: Ref<number>, scaleRef: Readonly<ShallowRef<HTMLDivElement | null>>) {
    // 响应式数据
    const lengthInPixel = ref<string | undefined>();
    let animateFrame: Processing | undefined;

    // 监听右侧位置变化
    watch(navigatorScaleRight, (val) => {
        if (scaleRef.value) {
