...
onMounted(() => {
  ...
  let dispose = xbsjEarthUi.activeViewer?.editingEvent.disposableOn(
    (val) => val?.type === 'end' && (currentMode.value = '')
  )
  let dispose2 = xbsjEarthUi.activeViewerChanged.disposableOn((val) => {
    if (dispose) dispose()
    dispose = xbsjEarthUi.activeViewer?.editingEvent.disposableOn(
      (val) => val?.type === 'end' && (currentMode.value = '')
    )
  })
  sceneTree &&
    xbsjEarthUi.d(
      sceneTree.selectedItems.changedEvent.don((val) => {
...
const iconColor = (item: any, index: number) =>
  !item.allowEditing
    ? checkColor.indeterminate
    : currentMode.value === item.type || hoverIndex.value === index
      ? checkColor.checked
      : checkColor.default
...
<template>
  <div class="table">
    <div class="row">
      <div
        class="cell"
        v-for="(item, index) in enditingList"
        :key="item.name"
        :title="item.name"
        @click="changeCurrentMode(item)"
        @mouseenter="hoverIndex = index"
        @mouseleave="hoverIndex = -1"
        :class="{ 'disabled-element': !item.allowEditing }"
      >
        <es-icon :name="item.icon" :color="iconColor(item, index)" :size="16" />
