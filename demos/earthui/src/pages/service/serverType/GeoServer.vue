...
const ok = () => {
  ...
  const format =
    currentItem.value.urls[0].format === 'application/openlayers'
      ? 'image/png'
      : currentItem.value.urls[0].format
  ...
  const options = {
    ...
    tilingScheme: {
      type:
        currentItem.value.tileMatrixSets[0].params.tilingScheme === 'Geographic'
          ? 'GeographicTilingScheme'
          : currentItem.value.tileMatrixSets[0].params.tilingScheme === 'WebMercator'
