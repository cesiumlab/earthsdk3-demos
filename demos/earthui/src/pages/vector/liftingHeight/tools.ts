  ...
  if (obj['sceneObj']) {
    ...
    if (sceneObject instanceof ESObjectWithLocation) {
      ...
      xbsjEarthUi.activeViewer
        .getHeightByLonLat(position[0], position[1], 'Visibility')
        .then((res) => {
          if (res) {
    ...
    if (sceneObject instanceof ESGeoLineString) {
      ...
      if (points && points.length > 0) {
        for (let index = 0; index < points.length; index++) {
          if (!xbsjEarthUi.activeViewer) return
          const res = await xbsjEarthUi.activeViewer.getHeightByLonLat(
            points[index][0],
            points[index][1],
            'Visibility'
          )
          const h = [points[index][0], points[index][1], Number(res) + Number(height)] as [
            number,
            number,
            number
          ]
