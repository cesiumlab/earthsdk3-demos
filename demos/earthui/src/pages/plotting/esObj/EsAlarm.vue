...
let sceneObject2: ESAlarm | undefined = undefined
...
const createSceneObject = (position: [number, number, number]) => {
  ...
  sceneObject2 = xbsjEarthUi.createSceneObject(ESAlarm) as ESAlarm
...
const createOneSceneObject = () => {
  if (!selected.value) return
  sceneObject = xbsjEarthUi.createSceneObject(ESAlarm) as ESAlarm
  if (sceneObject) {
