<template>
  ...
  >
    <LabelInput
      v-model="onePolyNameId"
      :activeMode="onePolyId"
      :disabled="false"
      :label="'多边形1'"
      :checkbox="true"
      :list="onePolyList"
      :checkboxFun="changeOnePolygonShow"
      :liClickFun="changeOnePolygonId"
      :placeholder="'请选择多边形'"
    ></LabelInput>
    <LabelInput
      v-model="twoPolyNmaeId"
      :activeMode="twoPolyId"
      :disabled="false"
      :label="'多边形2'"
      :checkbox="true"
      :list="twoPolyList"
      :checkboxFun="changeTwoPolygonShow"
      :liClickFun="changeTwoPolygonId"
      :placeholder="'请选择多边形'"
    ></LabelInput>
  </PopList>
</template>
...
const ok = () => {
  ...
  if (result && result.status === 'included') {
    ...
    ElMessage.warning('无法计算这两个多边形的差异')
  }
}
const scene = () => {
