...
let sceneObject2: ESWidget | undefined = undefined
...
const select = (item: { mode: boolean; img: any; name: string }) => {
  //点击选择框中的部件按钮
  selected.value = item
  destroy()
...
const createSceneObject = (position: [number, number, number]) => {
  ...
  sceneObject2 = xbsjEarthUi.createSceneObject(ESWidget) as ESWidget
  sceneObject2.position = position
  sceneObject2.screenRender = selected.value.mode
