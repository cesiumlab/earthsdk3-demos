...
const getMaterialNameList = async () => {
  ...
  list.value = temp.map((item: any) => {
    ...
    return {
      key: item,
      value: sceneObject.materialOverrideMap ? sceneObject.materialOverrideMap[item] : undefined,
      select: false
    }
  })
}
...
onMounted(() => {
  ...
  nextTick(() => {
    const sceneObject = props.setStyleTreeItem?.sceneObject as ES3DTileset
    orginalMaterialOverrideMap.value = sceneObject.materialOverrideMap
      ? JSON.parse(JSON.stringify(sceneObject.materialOverrideMap))
      : {}
  })
})
</script>
