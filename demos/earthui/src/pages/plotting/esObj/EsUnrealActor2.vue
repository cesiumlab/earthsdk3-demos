<template>
    <PopList :title="'绑定场景对象'" :showButton="true" @ok="ok">
        <div>
            ...
            <LabelInput v-model="actorTag" :label="'actorTag'" :checkbox="true" :list="option"
                :liClickFun="changeActorTag" :placeholder="'请填入或者选择actorTag'" :listContent="'value'"></LabelInput>
...
const ok = () => {
    ...
    viewer.getObjectByInfo({ actorTag: actorTag.value, componentTag: undefined }).then(res => {
        ...
        if (!res || !res.object) {
            ...
            if (!lastSceneTreeItem) {
                ...
                if (lastSceneTreeItem.type === 'Folder') {
                    treeItem = sceneTree.createSceneObjectTreeItem(res.object.type, undefined, lastSceneTreeItem, 'Inner')
                } else {
                    treeItem = sceneTree.createSceneObjectTreeItem(res.object.type, undefined, lastSceneTreeItem, 'After')
                }
            }
...
//生成id
const getGrid = () => {
    var grid = "xxxxxxxxxxxx4xxxyxxxxxxxxxxxxxxx".replace(
        /[xy]/g,
        function (c) {
