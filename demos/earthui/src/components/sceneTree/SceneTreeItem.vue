...
onMounted(() => {
  ...
  if (sceneTreeItem.sceneObject instanceof ES3DTileset) {
    ...
    let tilesUrl =
      typeof sceneTreeItem.sceneObject.url === 'string'
        ? sceneTreeItem.sceneObject.url
        : sceneTreeItem.sceneObject.url.url
...
const getItemStyle = () => ({
  background:
    sceneTreeItem.uiTreeObject.selected && isLastSelectedObject
      ? 'rgba(27, 27, 27, 0.5)'
      : sceneTreeItem.uiTreeObject.selected
        ? 'rgba(27, 27, 27, 0.5)'
        : hover.value
          ? 'rgba(27, 27, 27, 0.5)'
          : '',
...
const collapseFunc = () =>
  (sceneTreeItem.uiTreeObject.collapsed = !sceneTreeItem.uiTreeObject.collapsed)
...
const createTreeItem = (item: SceneTreeItem, list?: TileTreeItem[]) => {
  ...
  treeItems.forEach((el) => {
    item.insertNewTreeItem('Inner', el)
  })
}
