...
const saerct = (e?: any) => {
  ...
  getNoToken(
    `https://restapi.amap.com/v3/place/text?key=2d3c6326ef327eecba326ac5780fcab1&keywords=${name.value}`
  )
    .then((res: any) => {
...
<template>
  <div class="search_search">
    ...
    <div class="search_search_input">
      ...
      <span @click="cleanName"
        ><es-icon :name="'guanbi'" :color="'rgba(230, 230, 230, 1)'" :size="12"
      /></span>
    </div>
    ...
    <ul>
      <li
        ...
        @click="flyToNmae(item, index)"
        :class="{ active: liActive === index }"
        :title="`${item.name}-${item.adname}-${item.address}`"
      >
