...
const getCollapseClass = () => {
    try {
        if ((!sceneTreeItem.children || (sceneTreeItem.children && sceneTreeItem.children.length === 0)) && sceneTreeItem.type !== 'ES3DTileset') return 'collapse-box'
    } catch (error) {
...
<template>
    <div :class="['item_style', 'tile_item']" :style="getItemStyle()" @dblclick="flyTo()">
        ...
        <div :style="itemTopStyle()" class="top_style">
            ...
            <!-- 文件夹类型图标 -->
            <span class="scene-tree-item-type"
                :class="sceneTreeItem.extras.type === `node` ? 'node-type' : 'element-type'"></span>
